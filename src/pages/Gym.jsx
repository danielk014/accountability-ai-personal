...
import { sendGymMessage, analyzeFoodWithAI } from "@/api/claudeClient";
...
function loadNutrition() {
  try {
    const raw = _readStorage(getNutritionKey());
    return raw ? JSON.parse(raw) : { logs: [] };
  } catch { return { logs: [] }; }
}

function saveNutrition(data) {
  supabaseStorage.setItem(getNutritionKey(), JSON.stringify(data));
}
...
function NutritionTab({ nutrition, onUpdate }) {
  ...
  const [aiImage, setAiImage] = useState(null); // { data, mediaType, preview }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <GymDatePicker selectedDate={selectedDate} onSelect={setSelectedDate} />
        <button onClick={() => navigateDay(1)} className="p-2 rounded-xl hover:bg-slate-100 text-slate-500 transition">
...
export default function Gym() {
  ...
  function handleNutritionUpdate(patch) {
