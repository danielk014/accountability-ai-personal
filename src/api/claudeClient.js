...
is_processed_meat: true if this food IS or CONTAINS processed meat (bacon, hot dog, sausage, salami, pepperoni, deli meat, ham, jerky, corned beef, prosciutto, bratwurst, kielbasa, bologna, pastrami, spam, canned meat, luncheon meat, or any meat that has been smoked, cured, salted, or had preservatives added). false otherwise.
...
disease_risks: Array of disease risk objects for this specific food. Each object: {"disease":"disease name","risk":"high/moderate/low","reason":"why this food contributes"}. Focus on SERIOUS diseases: cancers (colorectal, pancreatic, bladder, stomach — from processed meat/nitrites/PAHs, WHO Group 1/2A), cardiovascular disease (trans fats, arterial damage), liver cancer & cirrhosis (alcohol), kidney disease (chronic high sodium), neurodegeneration/dementia (alcohol neurotoxicity, heavy metals). Do NOT include obesity, type 2 diabetes, or hypertension — the user is lean and athletic. Only include diseases with genuine scientific evidence linking this food. Empty array [] for clean whole foods.
...
ch_trigger_reason: if trigger is true, brief explanation why. Empty string if not a trigger.`;
  ...
  return {
    ...
    isProcessedMeat: !!p.is_processed_meat,
    additivesWarning: p.additives_warning || "",
    healthNote:    p.health_note || "",
    diseaseRisks:  (Array.isArray(p.disease_risks) ? p.disease_risks : []).filter(d => !/obesity|diabetes|hypertension/i.test(d.disease)),
